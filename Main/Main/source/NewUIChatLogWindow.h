...
namespace SEASON3B
{
	...
	private:
		...
		POINT	m_WndPos, m_ScrollBtnPos, m_NewWndPos;
		SIZE	m_WndSize;
	...
	public:
		...
		bool m_bShowFrame;
		int m_ShowSystemMessage;
		int m_ShowWhisperMessage;
		int m_ShowGuildMessage;
		int m_ShowPartyMessage;

	protected:
		type_vector_msgs* GetMsgs(MESSAGE_TYPE MsgType);
		type_vector_msgs* GetMsgsSystem(MESSAGE_TYPE MsgType);
		void ProcessAddText(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, DWORD MsgColor, MESSAGE_TYPE ErrMsgType);
