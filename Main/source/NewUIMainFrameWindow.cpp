...
void SEASON3B::CNewUIMainFrameWindow::SetButtonInfo()
{
	...
	m_BtnCShop.ChangeButtonInfo(frame_cx + 530, y_Next, x_Add, y_Add);
...
void SEASON3B::CNewUIMainFrameWindow::RenderGuageSD()
{
	...
	width = 16.f, height = 53.f;
	x = 118 + frame_cx; y = 580.f - -52.f;
	fY = y + (fShield * height);
	fH = height - (fShield * height);
	fV = fShield;

	glColor3f(1.0, 0.94999999, 0.75);
	RenderBitmap(IMAGE_GAUGE_SD, x, fY, width, fH, 0.f, fV * height / 64.f, width / 18.f, (1.0f - fV) * height / 70.f);
	glColor3f(0.91000003, 0.81, 0.60000002);
	SEASON3B::RenderNumber(x + 18, 680 - 19, (int)wShield);
	glColor4f(1.f, 1.f, 1.f, 1.f);
