...
void CCommandManager::Init() // OK
{
	...
	this->Add("/drop", COMMAND_DROP);
	this->Add("/limparinv", COMMAND_CLEARINV);
	this->Add("/readd", COMMAND_READD);
...
{
	int qtd = 0, section, type, level, skill, luck, option, exc, set;
	//if(gServerInfo.m_CommandMakeSwitch == 0)
	//{
	//	return;
	//}
...
void CCommandManager::CommandClearInventory(LPOBJ lpObj, char* arg) // OK
{
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Inventário limpido");
...
void CCommandManager::CommandReAdd(LPOBJ lpObj, char* arg)
{
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Pontos Resetados!");
...
void CCommandManager::CommandTradeX(LPOBJ lpObj, char* arg)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Você está ocupado no momento!");
		return;
	}
	...
	if (lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "O jogador alvo não está online");
		return;
	}
