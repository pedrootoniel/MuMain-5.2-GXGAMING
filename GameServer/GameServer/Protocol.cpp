...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	//LogAdd(LOG_BLUE, "Protocol: [0x%02X] [0x%02X]", head, ((lpMsg[0] == 0xC1) ? lpMsg[3] : lpMsg[4]));
		...
		case 0xF3:
			switch(lpMsg[3])
			{
				...
				case 0x53:
					...
					break;
				case 0xFA:
					CGClientKeyRecv((PMSG_ANTI_CLIENT_KEY_RECV*)lpMsg, aIndex);
					break;
				#ifdef TRADE_X
				case 0xC0:
...
void CGClientKeyRecv(PMSG_ANTI_CLIENT_KEY_RECV* lpMsg, int aIndex)
{
	if (OBJECT_RANGE(aIndex) == 0) 
	{
		return;
	...
	if (_strcmpi(lpMsg->m_Language, "Por") == 0) 
	{
		strcpy_s(lpMsg->m_Language, "Por");
	}

	if (_strcmpi(lpMsg->m_Language, "Eng") == 0) 
	{
		strcpy_s(lpMsg->m_Language, "Eng");
	}

	if (_strcmpi(lpMsg->m_Language, "Spn") == 0) 
	{
		strcpy_s(lpMsg->m_Language, "Spn");
	}

	if (_strcmpi(lpMsg->m_Language, "Por") != 0 && _strcmpi(lpMsg->m_Language, "Eng") != 0 && _strcmpi(lpMsg->m_Language, "Spn") != 0)
	{
		std::memcpy(lpMsg->m_Language, "Por", sizeof(lpMsg->m_Language));
	}

	strcpy_s(gObj[aIndex].m_Language, lpMsg->m_Language);
}
...
void CGRegisterAccountRecv(PMSG_REGISTER_ACCOUNT_RECV* lpMsg, int aIndex)
{
	...
	if (lpObj->LoginMessageSend == 0)
	{
		...
		LogAdd(LOG_BLUE, "%s - %s - %s - %s", account, password, PersonalCode, PersonalEmail);
