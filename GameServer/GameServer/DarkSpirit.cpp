...
void CDarkSpirit::Set(int aIndex, CItem* lpItem) // OK
{
	...
	if (lpItem->m_Index != GET_ITEM(13, 5) && !gCustomDarkSpirit.checkIsDarkSpirit(lpItem->m_Index)) // Dark Spirit
	{
		return;
	}
...
{
		this->m_AttackDamageMin = gServerInfo.m_DarkSpiritAttackDamageMinConstA + (lpItem->m_PetItemLevel * gServerInfo.m_DarkSpiritAttackDamageMinConstB) + ((lpObj->Dexterity + lpObj->AddDexterity) / gServerInfo.m_DarkSpiritAttackDamageMinConstC);

		this->m_AttackDamageMax = gServerInfo.m_DarkSpiritAttackDamageMaxConstA + (lpItem->m_PetItemLevel * gServerInfo.m_DarkSpiritAttackDamageMaxConstB) + ((lpObj->Dexterity + lpObj->AddDexterity) / gServerInfo.m_DarkSpiritAttackDamageMaxConstC);

		this->m_AttackSpeed = gServerInfo.m_DarkSpiritAttackSpeedConstA + ((lpItem->m_PetItemLevel * gServerInfo.m_DarkSpiritAttackSpeedConstB) / gServerInfo.m_DarkSpiritAttackSpeedConstC) + ((lpObj->Dexterity + lpObj->AddDexterity) / gServerInfo.m_DarkSpiritAttackSpeedConstD);
...
void CDarkSpirit::SetMode(eDarkSpiritMode mode, int aIndex) // OK
{
	...
	if (lpObj->Class != CLASS_DL && !gCustomDarkSpirit.checkIsDarkSpirit(lpObj->Inventory[236].m_Index))
	{
		return;
	}
...
#pragma region DAMAGE_CALC
	...
	//gAttack.DamageSlotPrimarySprite(lpTarget, damage);		// Slot 1
	////
	//gAttack.DamageSlotSecondarySprite(lpTarget, damage);	// Slot 2
...
void CDarkSpirit::DarkSpiritSprite(LPOBJ lpObj, int damage) // OK
{
	...
	CItem* pItem = &lpObj->Inventory[236];
	...
	if (gCustomDarkSpirit.checkIsDarkSpirit(pItem->m_Index))
	{
		if (!gCustomDarkSpirit.checkIsDarkSpirit(pItem->m_Index))
		{
			return;
		...
		//MaxDurSmall = (MaxDurSmall * lpObj->PetDurabilityRate) / 100;
		//
		//if (pItem->m_DurabilitySmall >= MaxDurSmall)
		//{
		//	pItem->m_Durability = (((--pItem->m_Durability) < 1) ? 0 : pItem->m_Durability);
		//	pItem->m_DurabilitySmall = 0;
		//
		//	if (pItem->CheckDurabilityState() != 0)
		//	{
		//		gObjectManager.CharacterCalcAttribute(lpObj->Index);
		//	}
		//
		//	gItemManager.GCItemDurSend(lpObj->Index, 1, (BYTE)pItem->m_Durability, 0);
		//
		//	GCPetItemInfoSend(lpObj->Index, 0, 0, 1, pItem->m_PetItemLevel, pItem->m_PetItemExp, (BYTE)pItem->m_Durability);
		//}
	}
}
