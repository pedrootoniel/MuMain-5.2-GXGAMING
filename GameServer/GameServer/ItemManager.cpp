...
bool CItemManager::CheckItemMoveToInventory(LPOBJ lpObj,CItem* lpItem,int slot) // OK
{
	...
	if (lpItem->m_Slot < 240 || (lpItem->m_Durability == 255))
	{
		{
	...
	if (lpItem->m_Slot >= 0 && lpItem->m_Slot <= 8 && slot >= 240 && slot <= 248)
	{
		int s_slot = lpItem->m_Slot + 240;

		if (s_slot == 240 && slot == 241)
		{
			return 1;
		}
...
BYTE CItemManager::MoveItemToInventoryFromTradeX(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	if (this->InventoryAddItem(lpObj->Index, lpObj->TradeX[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
void CItemManager::GCItemChangeSend(int aIndex, BYTE slot) // OK
{
	...
	if ((slot >= 0 && slot <= 8) || (slot >= 240 && slot <= 248))
	{
		if (lpObj->Inventory[slot].IsItem() == 0)
		{
			int targetSlot = 0;
