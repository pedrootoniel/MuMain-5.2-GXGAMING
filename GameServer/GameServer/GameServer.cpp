		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_ABOUT:
					...
					break;
				case IDM_ONLINEUSER:
					DialogBox(hInst, (LPCTSTR)IDD_DCACCOUNT, hWnd, (DLGPROC)UserOnline);
					break;
				case IDM_EXIT:
...
LRESULT CALLBACK UserOnline(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) // OK
{
	...
	case WM_INITDIALOG:
		...
		if (!hWndComboBox)
		{
			MessageBox(hDlg, "Could not create the combo box", "Failed Control Creation", MB_OK);
		...
		case IDC_BUTTONDC:
			...
			for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
			{
				if (gObj[n].Connected >= OBJECT_LOGGED && strcmp(gObj[n].Account, s) == 0)
				{
					...
					MessageBox(hDlg, "Account sucessfully disconnect!", "Confirm", MB_OK);
